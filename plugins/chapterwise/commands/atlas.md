...
```bash
# Read root index to understand project structure
cat index.codex.yaml
```
...
Full output format for each section type, following the Codex node schema.
...
The `atlases` array in the project root index is how ChapterWise.app, the VS Code extension, and the Reader Recipe discover and render atlases. Each atlas displays in a dedicated section separate from the manuscript content tree.
...
Follow the Language Guide for all user-facing messages.

**Atlas phases and their cooking verbs:**
...
**Rules:**
- Cooking verb + data noun: "Synthesizing character arcs... 14 characters." Never just "Synthesizing..."
- Include real numbers alongside every cooking verb
- Never say "recipe" to the user — it is an internal concept
- Never use theatrical completion lines
- No emojis
- Keep progress messages to 5–15 words
- "Done" does not need a cooking word — plain is better
